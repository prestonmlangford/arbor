...
def train(path):
    ...
    mse_p = metrics.mean_squared_error(sigmoid(y_test), sigmoid(y_pred))
    ...
    print(f"acc = {acc:.3f}")
    print(f"mse = {mse:.3f}")
    print(f"mse_p = {100*math.sqrt(mse_p):.3f}")
    
    print(" i: importance, value")
    for i,(m,s,c) in enumerate(zip(result.importances_mean,result.importances_std,reg.coef_)):
        coef.append(c)
